        ...
        <Layout>
          ...
          {showModal && (
            <Modal onClose={closeModal}>
              ...
              <div className="order-cancle-modal-btn">
                <button className="order-cancle-modal--no" onClick={closeModal}>
                  No
                </button>
                <button
                  className="order-cancle-moal--yes"
          ...
          <div className="col-lg-4">
            {orders?.map((element, index) => (
              <div className="widget-area" key={element.id}>
                <div className="single-widgets widget_egns_recent_post mb-30 order">
                  <div></div>
                  <span className="widget-title order-header">
                    <h3>{element.status}</h3>
                    {element.status === "Waiting for confirm" ? (
                      <button
                        className="order-cancle"
                        onClick={openModal.bind(
                          null,
                          element.id ? element.id : orders.length + 1
                        )}
                      >
                        <h4 className="order-cancle-btn"> Cancel Order</h4>
                      </button>
                    ) : (
                      ""
                    )}
                  ...
                  <div className="recent-post-wraper">
                    {Array.isArray(element.orderDetailDtoResponses) &&
                    ...
                    <table className={"order-table"}>
                      <tbody>
                        ...
                        <tr>
                          <th>Order's Name: </th>
                          <td>
                            {element.fullName ? element.fullName : "Guest"}
                          </td>
                        </tr>
                        <tr>
                          <th>Phone Number:</th>
